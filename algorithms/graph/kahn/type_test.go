...
func TestUnit_KahnCoherent(t *testing.T) {
	...
	graph.Add("c", "e")
	graph.Add("d", "e")
	test.NoError(t, graph.Build())
	result := graph.Result()
	test.True(t, len(result) == 5)
	test.Contains(t, []string{"a,b,c,d,e"}, strings.Join(result, ","))
}
...
func TestUnit_KahnCoherentBreakPoint(t *testing.T) {
	...
	test.NoError(t, graph.Build())
	result := graph.Result()
	test.True(t, len(result) == 4)
	test.Contains(t, []string{"a,b,c,d"}, strings.Join(result, ","))
}
