...
func TestUnit_Wrap(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Case4",
			args: args{msg: []error{Wrapf(New("err1"), "err1 message"),
...
func TestUnit_CauseUnwrap(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			v := Wrapf(tt.fields.cause, tt.fields.message)
			err := Cause(v)
			if (err != nil) != tt.wantErr {
...
func TestUnit_Trace(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := Trace(tt.err, "msg"); got != nil && !strings.Contains(got.Error(), tt.want) {
