...
func TestUnit_SimpleDI2(t *testing.T) {
	c := app.NewContainer(xc.New())
	test.NoError(t, c.Register(&SimpleDI1_A{A: "field A of struct"}))
	test.ErrorContains(t, c.Invoke(func(a *SimpleDI1_A) {
...
func TestUnit_DI_Default(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				test.ErrorContains(t, errs, tt.wantErrString)
			} else {
				test.NoError(t, errs)
			}
		})
